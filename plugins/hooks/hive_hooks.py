...
class HiveCliHook(BaseHook):
    ...
    def _get_user_pass(self):
        ...
        original_pass = self.run_as + str(timestamp) + salt
        sign = hashlib.sha1(original_pass.encode('utf-8')).hexdigest()
        encrypt_pass = "compass-" + str(timestamp) + "-" + sign  
    ...
    def _prepare_cli_cmd(self):
        ...
        if self.use_beeline:
            ...
            # 替换user为run as, 密码为加密
            user_pass = self._get_user_pass()
            user_name = self.run_as
            cmd_extra += ['-n', user_name]
            cmd_extra += ['-p', user_pass]
            # if conn.login:
